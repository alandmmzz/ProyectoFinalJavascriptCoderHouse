...
let subjects = [];
const subjectList = document.querySelector("ul#subjectList");
const subjectSelector = document.querySelector("select#taskSubject");
getSubjects();

async function getSubjects() {
  const response = await fetch("/js/subjects.json");
  const subjectsJSON = await response.json();
  subjects.push(...subjectsJSON)
  subjects.forEach((subject)=> {
  subjectList.innerHTML += `<li id="subject${subject.id}" class="list-group-item text-light d-flex justify-content-between align-items-center" style="background-color: ${subject.colour};">
                            ${subject.name}
